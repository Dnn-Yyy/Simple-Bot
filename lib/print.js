...
`.trim(),
    me + ' ~' + conn.user.name,
    timestamp.toLocaleString('id', { timeZone: 'Asia/Jakarta' }),
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) {
      for (let u of m.mentionedJid) {
        log = log.replace('@' + u.split`@`[0], chalk.blueBright('@' + (await conn.getName(u))))
      }
    }

    console.log(m.error != null ? chalk.red(log) : chalk.yellow(log))
  }
